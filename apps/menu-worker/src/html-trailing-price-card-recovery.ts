...
const EXPLICIT_A_LA_CARTE_SCOPE =
...
function parsedAmount(
  wholeValue: string,
  decimalsValue: string | undefined,
): number | null {
...
function looksLikeDishTitle(value: string): boolean {
  ...
  if (!title || title.length < 2 || title.length > 160 || !/\p{L}/u.test(title))
    return false;
...
function sectionHintBeforeTitle(
...
function candidatesInExplicitAlaCarteScope(
  ...
  const scopeStart = lines.findIndex((line) =>
    EXPLICIT_A_LA_CARTE_SCOPE.test(line),
  );
  if (scopeStart < 0) return candidates;

  const relativeScopeEnd = lines
    .slice(scopeStart + 1)
    .findIndex((line) => NEXT_MENU_SCOPE.test(line));
  const scopeEnd =
    relativeScopeEnd < 0 ? lines.length : scopeStart + 1 + relativeScopeEnd;
...
function parseNumberedMenuTitle(value: string): ParsedNumberedMenuTitle | null {
  ...
  if (!Number.isInteger(menuIndex) || menuIndex < 1 || menuIndex > 300)
    return null;
  if (!name || !/\p{L}/u.test(name) || CURRENCY_ONLY_NUMBERED_TITLE.test(name))
    return null;
...
export function recoverTrailingPriceCardHtmlItems(
  ...
  for (
    ...
    const locallyStrongStructuredTitle = Boolean(
      structuredCandidate &&
        isStrongLocalStructuredLeadingTitle(structuredCandidate),
    );
    const structuredLeadingTitle =
      structuredCandidate &&
      (useStructuredLeadingLayout || locallyStrongStructuredTitle)
        ? structuredCandidate
        : null;
    if (numberedTitle) {
    ...
    if (endpoint.residual && titlePosition !== pricePosition)
      descriptionParts.push(endpoint.residual);
    const description =
    ...
    candidates.push({
      item: {
        ...
        confidence: locallyStrongStructuredTitle ? 0.99 : 0.95,
        sourceExcerpt: lines
  ...
  const deduplicatedCandidates = preserveDocumentedDuplicateSections(
    scopedCandidates,
  ).map((item) => ({
    item,
    sectionHint: item.sectionName,
  }));
  const numberedCandidates = canonicalizeStrongNumberedMenu(
