...
describe("PDF source scope", () => {
  it("excludes generic beverage sections and resumes at a later dessert section", () => {
    ...
    expect(PDF_SOURCE_EXTRACTOR_VERSION).toBe("pdf-text-v37");
  ...
  it("treats labeled allergen rows as metadata between a PDF dish title and separate price", () => {
    const lines = [
      "DAGENS UTVALGTE OSTER",
    ...
    const visibleText = lines.join("\n");
    const parsed = extractMenuItemsFromPdfLines(lines);
    const recovered = recoverExplicitLowPerItemPdfRows(visibleText, parsed);
    const scoped = scopePdfMenuItems(visibleText, recovered);
