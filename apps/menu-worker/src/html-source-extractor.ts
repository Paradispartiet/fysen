...
function isPrefixedMainCourseSectionLabel(value: string): boolean {
  return PREFIXED_MAIN_COURSE_SECTION_LABEL.test(normalizeVisibleLine(value));
}

function isPlainFoodSectionLabel(value: string): boolean {
  const normalized = normalizeVisibleLine(value);
  return (
    FOOD_SECTION_LABEL.test(normalized) ||
    PREFIXED_MAIN_COURSE_SECTION_LABEL.test(normalized)
  );
}
...
function foodScopedText(lines: readonly string[]): FoodScopedText {
  ...
  for (const line of lines) {
    ...
    if (blockedHeadingLevel !== null) {
      if (!isPrefixedMainCourseSectionLabel(line)) continue;
      blockedHeadingLevel = null;
    }
