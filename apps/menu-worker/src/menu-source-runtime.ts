...
const BATCH_INTAKE_NON_FOOD_SECTION =
  /^(?:les\s+før\s+bestilling|read\s+before\s+ordering)$/iu;
...
export function isCanonicalStructuredMenuItem(
  item: MenuObservedItem,
): boolean {
  const filterName = item.name.trim().replace(/\p{Cf}/gu, "").trim();
  return (
...
export function isCanonicalBatchIntakeMenuItem(
  ...
  return (
    ...
    !BATCH_INTAKE_NON_FOOD_SECTION.test(sectionName)
  );
}

function mergeMissingRecoveredItems(
  items: readonly MenuObservedItem[],
  recovered: readonly MenuObservedItem[],
): readonly MenuObservedItem[] {
  if (recovered.length === 0) return items;
  const output = [...items];
  const existingNames = new Set(items.map((item) => item.normalizedName));

  for (const candidate of recovered) {
    if (existingNames.has(candidate.normalizedName)) continue;
    output.push(candidate);
    existingNames.add(candidate.normalizedName);
  }
...
export async function extractMenuSource(
  ...
  if (sourceType === "html" || sourceType === "json_ld") {
    ...
    const headingDominatesTrailingRecovery =
      shouldPreferDominantHeadingRecovery(
        headingPriceItems.length,
        trailingPriceCardItems.length,
        recoveredItems.length,
    ...
    const normalizedItems =
      extracted.method === "html_heuristic"
        ...
        : priceEnrichedItems;
    const canonicalItems = normalizedItems.filter(
      extracted.method === "json_ld"
        ? isCanonicalStructuredMenuItem
        : isCanonicalHtmlMenuItem,
    );
