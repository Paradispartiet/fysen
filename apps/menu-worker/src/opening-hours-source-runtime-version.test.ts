...
describe("opening-hours runtime extractor version", () => {
  it("includes canonical source extraction, duplicate recovery, scope priority and marker normalization semantics", () => {
    expect(OPENING_HOURS_RUNTIME_EXTRACTOR_VERSION).toBe(
      `${OPENING_HOURS_SOURCE_EXTRACTOR_VERSION}+${OPENING_HOURS_DUPLICATE_SECTION_RECOVERY_VERSION}+${OPENING_HOURS_SCOPE_HINT_RESOLVER_VERSION}+${OPENING_HOURS_MARKER_NORMALIZER_VERSION}`,
    );
    expect(OPENING_HOURS_RUNTIME_EXTRACTOR_VERSION).toBe(
      "hours-visible-v15+scope-duplicates-v1+scope-priority-v1+hours-marker-v3",
    );
  });
});
