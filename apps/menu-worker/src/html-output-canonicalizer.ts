...
function hasExplicitCardPricedNameProvenance(
  item: MenuObservedItem,
): boolean {
  if (item.priceMinor === null || item.priceMinor % 100 !== 0 || !item.sourceExcerpt)
    return false;
  const parts = item.sourceExcerpt
    .split(SOURCE_EXCERPT_SEPARATOR)
    .map((part) => part.trim())
    .filter(Boolean);
  if (parts.length < 2) return false;
  const first = normalizeDishName(parts[0] ?? "");
  if (first !== item.normalizedName) return false;

  const kroner = String(item.priceMinor / 100);
  const prefixed = new RegExp(
    `^(?:(?:NOK|kr\\.?)\\s*)${kroner}(?:[.,]\\d{1,2})?(?:\\s*,?[-–—])?import { createMenuItemSourceKey, normalizeDishName, type MenuObservedItem } from "@fysen/menu-core";
...
,
  ...
  const suffixed = new RegExp(
    `^${kroner}(?:[.,]\\d{1,2})?\\s*(?:,-|kr\\.?|NOK)import { createMenuItemSourceKey, normalizeDishName, type MenuObservedItem } from "@fysen/menu-core";
...
,
  ...
  return parts.slice(1).some((part) => prefixed.test(part) || suffixed.test(part));
}

function isDailyDessertNoise(item: MenuObservedItem): boolean {
  if (!DAILY_DESSERT_PLACEHOLDER.test(item.name.trim())) return false;
  return (
    !hasDirectPricedNameProvenance(item) &&
    !hasExplicitCardPricedNameProvenance(item)
  );
}
...
export function canonicalizeHtmlOutputItems(
  ...
  const labelFilteredItems = provenanceResolvedItems.filter(
    (item) => !isOutputNoiseLabel(item) && !isDailyDessertNoise(item),
  );
