...
The first exact-head proof after address resolution requested nine candidates and generated seven immediately. Festningen Restaurant (24 items), FYR Bistronomi & Bar (3), Kaffistova (21), Rorbua (19), Gamle Raadhus Restaurant (18), Frognerseteren Finstua (32) and Bristol Grill (26) all generated candidate manifests. Madonna's HTTP extraction recovered only one unique priced dish despite the published page visibly containing a larger menu, so the seed now re-proves the same first-party URL through the supported rendered-browser path. Le Benjamin returned a deterministic robots-policy refusal and is removed from intake rather than bypassing that policy.
