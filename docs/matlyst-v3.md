...
Matlyst er Fysens oppdagelseslag for mat. Produktet har to parallelle innganger som begge ender i den samme ferske Oslo-indeksen:

1. **Hva frister?** → brukerintensjon → canonical rett → vanlige Fysen-treff.
2. **Matkulturer** → kjøkken → canonical rett → vanlige Fysen-treff.
...
1. **Hva frister?** – lystinnganger som pizza, curry, nudler og grill.
2. **Alle aktive kjøkken i Matlyst** – én direkte, filtrerbar kjøkkenkatalog.

Katalogen kan filtreres på kjøkkennavn, kjøkkenets egne tradisjoner, canonical rett og alias. Eksempler er `japansk`, `ramen`, `momo` og `pierogi`. Kortene viser ikke verdensdel eller kulinarisk region.

Kjøkken sorteres med fersk Oslo-dekning først når browse-data er tilgjengelig. Live-signaler kommer fra `DishBrowseResponse`; de lager ikke en ny indeks og registreres ikke som brukersøk.
